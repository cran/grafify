test_that("Check post-hoc level-wise comparisons", {
  ...
  em1 <- posthoc_Levelwise(mmod5, c("Treatment", "Strain"))
  em2 <- emmeans(mmod5, pairwise ~Treatment|Strain, Ref = 1, adjust = "fdr")
  ...
  em3 <- posthoc_Levelwise(mmod6, c("Treatment", "Strain"))
  em4 <- emmeans(mmod6, pairwise ~Treatment|Strain, Ref = 1, adjust = "fdr")
